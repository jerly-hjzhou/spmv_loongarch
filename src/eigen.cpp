...
Eigen::SparseMatrix<float> readSparseMatrix(const std::string &filename) {
  std::ifstream file(filename);
  int rows, cols, nonZeros;
  file >> rows >> cols >> nonZeros;

  Eigen::SparseMatrix<float> sparseMatrix(rows, cols);
...
int main(int argc, char *argv[]) {
  ...
  __TIC__(READ_MATRIX)
  // 从matrix.txt读取稀疏矩阵
  Eigen::SparseMatrix<float> sparseMatrix = readSparseMatrix(matrixFile);
  __TOC__(READ_MATRIX)
