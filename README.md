...
## 效果展示
我们测试性能的环境是4线程的龙芯3C5000。如下图所示，该图横坐标表示算子的执行时间，横坐标是进行向量乘的稀疏矩阵名称。优化后的算法相比原始的spmv算法在大部分数据上都取得了1.5以上的加速比。
![speed](./pic/speedup.png)

下图是优化后的算法和原始算法的SPMV核心计算部分的比较，纵坐标表示相较于原始SPMV算法的加速比，横坐标是进行向量乘的稀疏矩阵名称。
![exec](./pic/exec.png)
